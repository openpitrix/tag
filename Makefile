...
build-image-%: ## build docker image
	@if [ "$*" = "latest" ];then \
	docker build -t openpitrix/tag:latest .; \
	docker build -t openpitrix/tag:flyway -f ./pkg/db/Dockerfile ./pkg/db/; \
	elif [ "`echo "$*" | grep -E "^v[0-9]+\.[0-9]+\.[0-9]+"`" != "" ];then \
	docker build -t openpitrix/tag:$* .; \
	docker build -t openpitrix/tag:flyway-$* -f ./pkg/db/Dockerfile ./pkg/db/; \
	fi

push-image-%: ## push docker image
	@if [ "$*" = "latest" ];then \
	docker push openpitrix/tag:latest; \
	docker push openpitrix/tag:flyway; \
	elif [ "`echo "$*" | grep -E "^v[0-9]+\.[0-9]+\.[0-9]+"`" != "" ];then \
	docker push openpitrix/tag:$*; \
	docker push openpitrix/tag:flyway-$*; \
	fi
